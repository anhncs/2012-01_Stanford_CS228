...
%   There is no penalty for submitting, so go ahead and try this!
  ...
  for s = 1:numel(submitParts)
    ...
    % Hack for part 11 extra credit
    if (samplePartId == 11)
        partIdsToSubmit  = 11;
    else
        partIdsToSubmit = [samplePartId testPartId];
    end
    
    for thisPartId = partIdsToSubmit
...
function srcs = sources()
  % Separated by part
  srcs = { { 'StochasticGradientDescent.m', 'partner.txt' }, 
           { 'LRSearchLambdaSGD.m', 'partner.txt' },
           { 'CliqueTreeCalibrate.m', 'partner.txt' },
           { 'InstanceNegLogLikelihood.m', 'partner.txt'},
           { 'InstanceNegLogLikelihood.m', 'partner.txt'},
           { 'ExtraCredit.m', 'partner.txt' }
         };
end
...
elseif partId == 10
    ...
    out = SerializeVector(grad);
elseif partId == 11
    if (~exist('ECPredictions.mat'))
        warning('You have not run SaveECPrediction.m before submitting the extra credit.');
        out = '';
        return;
    end
    load ECPredictions.mat;
    v = yourPredictions(:);
    v = [numel(v); v];
