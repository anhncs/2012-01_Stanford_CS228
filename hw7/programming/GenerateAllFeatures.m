...
function featureSet = GenerateAllFeatures(X, y, modelParams)

    unconditionedFFs = {@ComputeUnconditionedSingleFeatures, @ComputeUnconditionedPairFeatures};
    ...
    for i = 1:numConditionedFFs
        fn = conditionedFFs{i};
        conditionedCell{i} = fn(y, X, modelParams);
    end

    unconditionedCell = cell(1, numUnconditionedFFs);
    for i = 1:numUnconditionedFFs
        fn = unconditionedFFs{i};
        unconditionedCell{i} = fn(y, modelParams);
    end
