...
compactTree = ComputeInitialPotentials(cliqueTree);
...
[i j] = GetNextCliques(compactCliqueTree, messages);
...
finalPotentials = CliqueTreeCalibrate(compactCliqueTree, 0);
...
exactMarginals = ComputeExactMarginalsBP(factorList, [], 0);
...
factor = FactorMaxMarginalization(inputFactor, 7);
...
inputFactor = UnserializeFactorsFgOctave(auxstring);
factor = FactorMaxMarginalization(inputFactor, 22);
out = SerializeFactorsFg(factor);
...
finalPotentials = CliqueTreeCalibrate(compactCliqueTree, 1);
...
mapMarginals = ComputeExactMarginalsBP(factorList, [], 1);
...
mapDecoded = MaxDecoding(factorList);
