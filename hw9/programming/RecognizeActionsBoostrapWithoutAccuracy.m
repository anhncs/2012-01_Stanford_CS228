...
function predicted_labels = RecognizeActionsBoostrapWithoutAccuracy(datasetTrain, datasetTest, G, maxIter)
% Bootstrap Method
B = 10;
...
[P{3} loglikelihood3 ClassProb3 PairProb3] = EM_HMM(actionData3, datasetTrain(3).poseData, G, datasetTrain(3).InitialClassProb, datasetTrain(3).InitialPairProb, maxIter);
...
for i=2:B
    ...
    [Ptemp{3} loglikelihood3 ClassProb3 PairProb3] = EM_HMM(actionData3, datasetTrain(3).poseData, G, datasetTrain(3).InitialClassProb, datasetTrain(3).InitialPairProb, maxIter);
    
    if loglikelihood1(length(loglikelihood1)) < 1 && loglikelihood2(length(loglikelihood2)) < 1 && loglikelihood3(length(loglikelihood3)) < 1 ...
        && loglikelihood1(length(loglikelihood1)) > -100000 && loglikelihood2(length(loglikelihood2)) > -100000 && loglikelihood3(length(loglikelihood3)) > -100000
