...
for i = 1:size(E, 1),
    ...
    for j = 1:length(F),
        ...
        if (~isempty(indx)),
            ...
            %%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
            all_possible_assignment = IndexToAssignment([1:length(F(j).val)],F(j).card);
            % The following is the indices that having non-zeor value (in col format)
            indices = find(all_possible_assignment(:,indx) ~= x);
            F(j).val(indices) = 0*F(j).val(indices);
            %%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
