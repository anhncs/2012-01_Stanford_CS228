...
% You have brown hair right? So...
% Phenotype is Brown.
% Allele is a 'B'
% Genotype: If 'BB' and 'Bb' represents having brown hair and 'bb' is having blond hair then your genotype would be either 'BB' or 'Bb'.
...
for i=1:size(genotypesToAlleles,1)
    allele_pair = genotypesToAlleles(i,:);
    if( allele_pair(1) ~= allele_pair(2) )
        p = 2*alleleFreqs(allele_pair(1))*alleleFreqs(allele_pair(2));
    else
        p = alleleFreqs(allele_pair(1))^2;
    end
