...
function srcs = sources()
  % Separated by part
  srcs = { { 'phenotypeGivenGenotypeMendelianFactor.m', 'partner.txt' }, ...
           { 'phenotypeGivenGenotypeFactor.m', 'partner.txt' }, ...
           { 'genotypeGivenAlleleFreqsFactor.m', 'partner.txt' }, ...
           { 'genotypeGivenParentsGenotypesFactor.m', 'partner.txt' }, ...
           { 'constructGeneticNetwork.m', 'partner.txt' }, ...
           { 'phenotypeGivenCopiesFactor.m', 'partner.txt' }, ...
           { 'constructDecoupledGeneticNetwork.m', 'partner.txt' }, ...
           { 'constructSigmoidPhenotypeFactor.m', 'partner.txt' }
         };
end
...
function out = output(partId, auxstring)
  ...
  if partId == 1
    isDominantArray = UnserializeDominance(auxstring);
    genotypeVar = 1;
    phenotypeVar = 3;
    pggmf(1) = phenotypeGivenGenotypeMendelianFactor(isDominantArray(1), genotypeVar, phenotypeVar);
    pggmf(1) = SortFactorLaterVars(pggmf(1));
    pggmf(2) = phenotypeGivenGenotypeMendelianFactor(isDominantArray(2), genotypeVar, phenotypeVar);
    pggmf(2) = SortFactorLaterVars(pggmf(2));
    out = SerializeFactorsFg(pggmf);
  elseif partId == 2
    alphaListArray = UnserializeAlphasAlleleFreqs(auxstring);
    genotypeVar = 1;
    phenotypeVar = 3;
    pggf(1) = phenotypeGivenGenotypeFactor(alphaListArray(1,:)', genotypeVar, phenotypeVar);
    pggf(1) = SortFactorLaterVars(pggf(1));
    pggf(2) = phenotypeGivenGenotypeFactor(alphaListArray(2,:)', genotypeVar, phenotypeVar);
    pggf(2) = SortFactorLaterVars(pggf(2));
    out = SerializeFactorsFg(pggf);
  elseif partId == 3
    alleleFreqsArray = UnserializeAlphasAlleleFreqs(auxstring);
    genotypeVar = 1;
    ggaff(1) = genotypeGivenAlleleFreqsFactor(alleleFreqsArray(1,:)', genotypeVar);
    ggaff(2) = genotypeGivenAlleleFreqsFactor(alleleFreqsArray(2,:)', genotypeVar);
    out = SerializeFactorsFg(ggaff);
  elseif partId == 4
    alleleListArray = UnserializeAlleleLists(auxstring);
    genotypeVarChild = 3;
    genotypeVarParentOne = 1;
    genotypeVarParentTwo = 2;
    ggpgf(1) = genotypeGivenParentsGenotypesFactor(alleleListArray{1}, genotypeVarChild, genotypeVarParentOne, genotypeVarParentTwo);
    ggpgf(1) = SortFactorLaterVars(ggpgf(1));
    ggpgf(2) = genotypeGivenParentsGenotypesFactor(alleleListArray{2}, genotypeVarChild, genotypeVarParentOne, genotypeVarParentTwo);
    ggpgf(2) = SortFactorLaterVars(ggpgf(2));
    out = SerializeFactorsFg(ggpgf);
  elseif partId == 5
    alleleFreqs = [0.1; 0.9];
    alleleList = {'F', 'f'};
    alphaList = [0.8; 0.6; 0.1];
    pedigreeList = UnserializePedigree(auxstring);
    cgn = constructGeneticNetwork(pedigreeList{1}, alleleFreqs, alleleList, alphaList);
    cgn = [cgn, constructGeneticNetwork(pedigreeList{2}, alleleFreqs, alleleList, alphaList)];
    for i = 1:length(cgn)
  ...
  elseif partId == 6
    ...
    pgpaf(1) = phenotypeGivenCopiesFactor(alphaListArray(1,:)', alleleFreqsThree, genotypeVarMotherCopy, genotypeVarFatherCopy, phenotypeVar);
    pgpaf(1) = SortFactorLaterVars(pgpaf(1));
    pgpaf(2) = phenotypeGivenCopiesFactor(alphaListArray(2,:)', alleleFreqsThree, genotypeVarMotherCopy, genotypeVarFatherCopy, phenotypeVar);
    pgpaf(2) = SortFactorLaterVars(pgpaf(2));
  ...
  elseif partId == 7
    ...
    cdgn = constructDecoupledGeneticNetwork(pedigreeList{1}, alleleFreqsThree, alleleListThree, alphaListThree);
    cdgn = [cdgn, constructDecoupledGeneticNetwork(pedigreeList{2}, alleleFreqsThree, alleleListThree, alphaListThree)];
    for i = 1:length(cdgn)
        cdgn(i) = SortFactorLaterVars(cdgn(i));
    end
    out = SerializeFactorsFg(sortStruct(cdgn));
  elseif partId == 8
    alleleWeightsCases = UnserializeAlleleWeights(auxstring);
    phenotypeVar = 3;
    geneCopyVarParentOneList = [1; 2];
    geneCopyVarParentTwoList = [4; 5];
    cspf(1) = constructSigmoidPhenotypeFactor(alleleWeightsCases{1}, geneCopyVarParentOneList, geneCopyVarParentTwoList, phenotypeVar);
    cspf(1) = SortFactorLaterVars(cspf(1));
    cspf(2) = constructSigmoidPhenotypeFactor(alleleWeightsCases{2}, geneCopyVarParentOneList, geneCopyVarParentTwoList, phenotypeVar);
    cspf(2) = SortFactorLaterVars(cspf(2));
...
function alleleListArray = UnserializeAlleleLists(str)
% This function converts a string with a set of allele lists into a cell
% array of allele list cell arrays.

[tok, str] = strtok(str);
numAlleleLists = sscanf(tok, '%d');

alleleListArray = {};

for i = 1:numAlleleLists
  % Iterate through lists of allele
  [tok, str] = strtok(str);
  numAlleles = sscanf(tok, '%d');
  
  alleleNames = {};
  for j = 1:numAlleles
      % Iterate through alleles and put them into the array
      [tok, str] = strtok(str);
      an = sscanf(tok, '%s');
      alleleNames{j} = an;
  end  
  alleleListArray{i} = alleleNames;
end
end

function alleleWeightsCases = UnserializeAlleleWeights(str)
% This function unserializes allele weights for genes in test cases.

[tok, str] = strtok(str);
numCases = sscanf(tok, '%d');

alleleWeightsCases = {};

for i = 1:numCases
  % Iterate through cases and find the allele weights for the alleles for
  % each gene in each case
  currentAlleleWeights = {};
  [tok, str] = strtok(str);
  numGenes = sscanf(tok, '%d');
  numAlleles = [];
  for j = 1:numGenes
      % Iterate through genes and record the numbers of alleles
      [tok, str] = strtok(str);
      numAlleles(j) = sscanf(tok, '%d');
  end
  
  for j = 1:numGenes
      % Iterate through genes and record the allele weights
      alleleWeightsj = [];
      for k = 1:numAlleles(j)
          % Iterate through alleles and read in the weights
          [tok, str] = strtok(str);
          alleleWeightsj(k) = sscanf(tok, '%f');
      end
      currentAlleleWeights{j} = alleleWeightsj';
  end    
  alleleWeightsCases{i} = currentAlleleWeights;
end
end

function listArray = UnserializeAlphasAlleleFreqs(str)
% This function converts a list of lists of alphas or allele frequencies in 
% string format into a list of lists of alphas or allele frequencies in 
% float format

[tok, str] = strtok(str);
numListEntries = sscanf(tok, '%d'); % Number of entries in each list

[tok, str] = strtok(str);
numLists = sscanf(tok, '%d'); % Number of lists (same as number of train/test cases)

listArray = zeros(numLists, numListEntries);
for i = 1:numLists
    % Iterate through the lists of alphas/allele frequencies and put them 
    % into the array
    for j = 1:numListEntries
        % Iterate through entries in the alphas/allele frequency list
        [tok, str] = strtok(str);
        listArray(i, j) = sscanf(tok, '%f');
    end
end
end

function dominantFirstArray = UnserializeDominance(str)
% This function unserializes a list of dominant bit flags.

[tok, str] = strtok(str);
numDominanceFlags = sscanf(tok, '%d');

dominantFirstArray = [];

for i = 1:numDominanceFlags
  [tok, str] = strtok(str);
  dominantFirst = sscanf(tok, '%d');
  dominantFirstArray = vertcat(dominantFirstArray, dominantFirst);
end
end

function pedigreeList = UnserializePedigree(str)
% UnserializePedigree Returns a pedigree list unserialized from str
% Assumes that the names have no spaces in them.

[tok, str] = strtok(str);
numPedigrees = sscanf(tok, '%d');

for j = 1:numPedigrees
    % Serialize each pedigree
    [tok, str] = strtok(str);
    numPeople = sscanf(tok, '%d');

    pedigree.names = cell(numPeople, 1);
    pedigree.parents = zeros(numPeople, 2);

    for i = 1:numPeople
      [tok, str] = strtok(str);
      pedigree.names{i} = str;  
    end

    for i = 1:numPeople
      [tok, str] = strtok(str);
      parent1 = sscanf(tok, '%f');
      [tok, str] = strtok(str);
      parent2 = sscanf(tok, '%f');
      pedigree.parents(i, 1) = parent1;
      pedigree.parents(i, 2) = parent2;
    end 

    pedigreeList{j} = pedigree;
end
end
